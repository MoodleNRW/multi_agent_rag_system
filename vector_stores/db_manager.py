...
import subprocess
import sys
from vector_stores.weaviate_client import create_weaviate_client, ensure_weaviate_connection, check_weaviate_data, create_weaviate_schema
from vector_stores.retriever import create_retrievers, ensure_global_client, create_retrievers_with_client
...
@cl.action_callback("fix_faq_collection")
async def on_fix_faq_collection(action):
    """Handler für die Reparatur der FAQ-Collection."""
    await action.remove()
    ...
    try:
        ...
        if not ensure_weaviate_connection(temp_client):
            await cl.Message(content="⚠️ Keine Verbindung zur Weaviate-Datenbank möglich.").send()
            return
        
        # Überprüfe, ob die FAQ-Collection existiert
        collection_names = temp_client.collections.list_all(simple=True)
        
        if "FAQ" in collection_names:
            # Lösche die bestehende FAQ-Collection
            await cl.Message(content="🔄 Lösche die bestehende FAQ-Collection...").send()
            temp_client.collections.delete("FAQ")
            
            # Überprüfe, ob die Collection gelöscht wurde
            collection_names = temp_client.collections.list_all(simple=True)
            if "FAQ" not in collection_names:
                await cl.Message(content="✅ FAQ-Collection erfolgreich gelöscht.").send()
            else:
                await cl.Message(content="⚠️ FAQ-Collection konnte nicht gelöscht werden.").send()
                return
        
        # Erstelle die FAQ-Collection neu mit Vektorisierer
        await cl.Message(content="🔄 Erstelle die FAQ-Collection neu mit Vektorisierer...").send()
        
        # Erstelle die FAQ-Collection mit dem Schema
        faq_properties = [
            temp_client.collections.config.Property(name="question", data_type=temp_client.collections.config.DataType.TEXT),
            temp_client.collections.config.Property(name="answer", data_type=temp_client.collections.config.DataType.TEXT),
            temp_client.collections.config.Property(name="date", data_type=temp_client.collections.config.DataType.DATE),
            temp_client.collections.config.Property(name="approved", data_type=temp_client.collections.config.DataType.BOOLEAN)
        ]
        
        # Erstelle die Collection mit Vektorisierer
        faq_config = temp_client.collections.create(
            name="FAQ",
            properties=faq_properties,
            vectorizer_config=temp_client.collections.config.Configure.Vectorizer.text2vec_openai()
        )
        
        # Überprüfe, ob die Collection erstellt wurde
        collection_names = temp_client.collections.list_all(simple=True)
        if "FAQ" in collection_names:
            await cl.Message(content="✅ FAQ-Collection erfolgreich neu erstellt mit Vektorisierer.").send()
            
            # Überprüfe die Konfiguration
            faq_collection = temp_client.collections.get("FAQ")
            vectorizer_info = faq_collection.config.vectorizer
            await cl.Message(content=f"ℹ️ FAQ-Collection Vektorisierer: {vectorizer_info}").send()
            
            # Aktualisiere die Retriever
            await cl.Message(content="🔄 Aktualisiere die Retriever...").send()
            
            # Erstelle neue Retriever mit dem temporären Client
            chunks_retriever, summaries_retriever, quotes_retriever, faq_retriever = create_retrievers_with_client(temp_client)
            ...
            if faq_retriever:
                ...
                await cl.Message(content="✅ Retriever erfolgreich aktualisiert.").send()
            else:
                await cl.Message(content="⚠️ FAQ-Retriever konnte nicht erstellt werden.").send()
        else:
            await cl.Message(content="⚠️ FAQ-Collection konnte nicht erstellt werden.").send()
    
    except Exception as e:
        logger.error(f"Fehler bei der Reparatur der FAQ-Collection: {str(e)}", exc_info=True)
        await cl.Message(content=f"⚠️ Fehler bei der Reparatur der FAQ-Collection: {str(e)}").send()
    finally:
        # Schließe den Client nur, wenn er existiert und alle Operationen abgeschlossen sind
        if temp_client:
            try:
                logger.info("Schließe temporären Weaviate-Client...")
                temp_client.close()
                logger.info("Temporärer Weaviate-Client erfolgreich geschlossen.")
            except Exception as close_error:
                logger.error(f"Fehler beim Schließen des temporären Weaviate-Clients: {str(close_error)}")
